...
function cevasta_rucka_horizontala(duzina = 18.6, visina = 2.5, debljina = 0.8) {
  ...
  const bar = rotateY(
    Math.PI / 2,
    stranslate(0, 0, -visina - 1, cylinder({ radius: r, height: duzina, center: [0, 0, 0], segments: 16 })),
  );
...
function build_gue90(sl, sd, v, d, brp = 1) {
  ...
  parts.push(
    colorize(CLR_FRONT, stranslate(sl - d - mdf - hdf, -sd + 0.15, 0.3, scube(m1, sd - d - mdf - hdf - 0.2, dh))),
  );
  ...
  if (upperH > 0) {
    for (let i = 0; i < brv; i++) {
      parts.push(
        colorize(
          CLR_FRONT,
          stranslate(0.15 + i * (s / brv), -d - m1, frizider + 1 + c + m1 + 0.3, scube(s / brv - 0.3, m1, upperH)),
        ),
      );
    }
  }
  ...
  if (upperH > 0) {
    for (let i = 0; i < brv; i++) {
      parts.push(
        colorize(
          CLR_FRONT,
          stranslate(0.15 + i * (s / brv), -d - m1, vde + rerna + m1, scube(s / brv - 0.3, m1, upperH)),
        ),
      );
    }
  }
  ...
  if (upperH > 0) {
    for (let i = 0; i < brv; i++) {
      parts.push(
        colorize(
          CLR_FRONT,
          stranslate(0.15 + i * (s / brv), -d - mdf, vde + rerna + m1, scube(s / brv - 0.3, mdf, upperH)),
        ),
      );
    }
  }
