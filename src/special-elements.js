...
export function createSpanningRadnaPloca(idxA, idxB) {
  ...
  if (tA.r !== tB.r && idxA !== idxB) {
    showNotification('Elementi moraju biti na istom zidu!', 'error');
    return;
  }
...
export function createCornerRadnaPloca(idxA, idxB, cornerIdx) {
  ...
  function makeSideSlab(len) {
    ...
    try {
      addModuleGroup(
        state.plan.length - 1,
        buildKitchenModule('radna_ploca', entry.p, state.materials, state.settings, px, py, topZ, sideR),
      );
    } catch (e) {
      console.error('3D build failed for side slab', e);
    }
  }
  ...
  if (otherIdx === cornerIdx) {
    const sideLen = lss - d;
    if (sideLen <= 0) {
      showNotification('lss nije veci od d!', 'error');
      return;
    }
    ...
    const dx = otherEl.pos[0] - cornerEl.pos[0],
      dy = otherEl.pos[1] - cornerEl.pos[1];
    ...
    if (onMainWall) {
      ...
      try {
        ...
        console.error('3D build failed for main-wall slab', e);
      }
      renderPlanList();
...
export function createSpanningCokla(idxA, idxB) {
  ...
  const entry = {
    ime: 'cokla',
    p: { l: String(finalL), h: '9.5', debljina: '1.8' },
    pos: [startElement.pos[0], -46, 0],
    r: startElement.r,
  };
  state.plan.push(entry);
  try {
    addModuleGroup(
      state.plan.length - 1,
      buildKitchenModule('cokla', entry.p, state.materials, state.settings, entry.pos[0], -46, 0, entry.r),
    );
  } catch (e) {
    console.error('3D build failed', e);
  }
...
let _openInputModalImpl = (label, defaultVal, callback) => {};
export function setOpenInputModal(fn) {
  _openInputModalImpl = fn;
}
function _openInputModal(label, defaultVal, callback) {
  _openInputModalImpl(label, defaultVal, callback);
}
