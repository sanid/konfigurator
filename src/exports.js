...
export async function exportPdf() {
  ...
  try {
    ...
    for (const [mat, rows] of Object.entries(groups)) {
      ...
      doc.text(
        'Ukupno za ' +
          ...
          matCost.toFixed(2) +
          ' € (' +
          (matCost * 117).toLocaleString('sr-RS') +
          ' RSD)',
        15,
        startY - 5,
      );
    ...
    const grandKant = list.reduce((s, r) => {
      const k = calcKant(r.kant, r.L, r.W);
      return s + (k.k * state.prices.kant_k + k.K * state.prices.kant_K) * r.qty;
    }, 0);
    ...
    doc.text(
      'UKUPNA VRIJEDNOST PONUDE: ' + grandEur.toFixed(2) + ' € (' + (grandEur * 117).toLocaleString('sr-RS') + ' RSD)',
      15,
      startY + 15,
    );
...
export async function exportModuleMPR(planIdx) {
  ...
  if (!panel) {
    showNotification('Nema stranica za MPR!', 'warning');
    return;
  }
...
export async function exportClientPdf() {
  ...
  try {
    ...
    const W = 210,
      H = 297;
    const name = state.clientName || 'Projekat_Meco';
    ...
    doc.setFontSize(32);
    doc.text('MECO', W / 2, 60, { align: 'center' });

    doc.setTextColor(200, 200, 220);
    doc.setFontSize(14);
    doc.text('PONUDA ZA KUHINJSKI PROGRAM', W / 2, 75, { align: 'center' });

    doc.setDrawColor(79, 122, 255);
    doc.setLineWidth(0.5);
    doc.line(60, 85, W - 60, 85);
    ...
    doc.text(date, W / 2, 110, { align: 'center' });

    const canvas = document.getElementById('three-canvas');
    if (canvas) {
      doc.addImage(canvas.toDataURL('image/jpeg', 0.85), 'JPEG', 25, 125, W - 50, (W - 50) * 0.55);
    }

    doc.setTextColor(100, 100, 130);
    doc.setFontSize(8);
    doc.text('Generisano automatski — Meco Konfigurator 2026', W / 2, H - 15, { align: 'center' });

    // ── Page 2: Overview + Materials ───────────────────────────────────────────
    doc.addPage();
    ...
    doc.setFontSize(16);
    doc.text('PREGLED KUHINJE', 15, 18);

    doc.setFontSize(10);
    doc.setTextColor(80, 80, 120);
    doc.text(`${name} · ${state.plan.length} elemenata · ${date}`, 15, 26);

    let y = 38;
    doc.setFontSize(11);
    doc.setTextColor(42, 91, 232);
    doc.text('ELEMENTI:', 15, y);
    y += 7;
    ...
    state.plan.forEach((item, i) => {
      ...
      doc.text(`${i + 1}. ${item.ime.replace(/_/g, ' ')} — ${sVal}cm`, 20, y);
      y += 5;
    });
    ...
    doc.setFontSize(11);
    doc.setTextColor(42, 91, 232);
    doc.text('MATERIJALI:', 15, y);
    y += 7;

    doc.setFontSize(9);
    const matEntries = [
      { label: 'Front', def: state.materials.front },
      { label: 'Korpus', def: state.materials.korpus },
      { label: 'Radna ploča', def: state.materials.radna },
      { label: 'Čokla', def: state.materials.cokla },
    ];
    for (const { label, def } of matEntries) {
      if (y > H - 20) break;
      const colorHex = def.color || '#888888';
      const r = parseInt(colorHex.slice(1, 3), 16);
      const g = parseInt(colorHex.slice(3, 5), 16);
      const b = parseInt(colorHex.slice(5, 7), 16);
      doc.setFillColor(r, g, b);
      doc.roundedRect(20, y - 3.5, 5, 5, 1, 1, 'F');
      doc.setTextColor(30, 30, 50);
      doc.text(`${label}: ${def.name}${def.type === 'texture' ? ' (tekstura)' : ''}`, 28, y);
      y += 6;
    }
    ...
    for (const p of bd.panels) {
      doc.text(p.material, 20, y);
      doc.text(p.cost.toFixed(2) + ' €', W - 20, y, { align: 'right' });
      y += 8;
    }

    const rows = [
      ['Kantovanje (tanko)', bd.kantThin.toFixed(2) + ' €'],
      ['Kantovanje (debelo)', bd.kantThick.toFixed(2) + ' €'],
      ['Okov (okovi, vodilice, kvake...)', bd.hardware.toFixed(2) + ' €'],
    ];

    if (bd.laborPct > 0) rows.push(['Rad (' + bd.laborPct + '%)', bd.labor.toFixed(2) + ' €']);
    if (bd.marginPct > 0) rows.push(['Marža (' + bd.marginPct + '%)', bd.margin.toFixed(2) + ' €']);
    ...
    doc.text('UKUPNO:', 20, y);
    doc.text(bd.grandTotal.toFixed(2) + ' €', W - 20, y, { align: 'right' });
    y += 8;

    doc.setFontSize(11);
    doc.setTextColor(80, 80, 120);
    doc.text((bd.grandTotal * 117).toLocaleString('sr-RS') + ' RSD', W - 20, y, { align: 'right' });

    y += 15;
    doc.setFontSize(9);
    doc.setTextColor(120, 120, 150);
    doc.text('Cijene su informativne i ne uključuju montažu. RSD po kursu 1€ = 117 RSD.', 15, y);

    y += 20;
    doc.setFontSize(9);
    doc.setTextColor(30, 30, 50);
    doc.text('Detaljni spisak okova:', 15, y);
    y += 6;

    doc.autoTable({
      startY: y,
      head: [['Artikal', 'Komada', 'Cijena/kom', 'Ukupno']],
      body: bd.hardwareItems.map((i) => [i.name, i.qty, i.unitPrice.toFixed(2) + '€', i.total.toFixed(2) + '€']),
      styles: { fontSize: 8, cellPadding: 2 },
      headStyles: { fillColor: [220, 225, 248], textColor: [30, 60, 180], fontStyle: 'bold' },
    ...
    if (res?.success) showNotification('Ponuda PDF sačuvana!', 'success');
  } catch (err) {
    console.error('Client PDF error:', err);
    showNotification('Greška pri generisanju ponude PDF', 'error');
  }
}
